...
We find this value of $K$ using the no-arbitrage principle. Suppose at time $t=0$, I borrow at risk free to buy a stock at $S_0$. My borrowing cost is subsidized by the dividend yield of the stock $q$. I also enter a short forward position with expiration $t=T$ and strike $K$, at a \$0 premium. 

This would suppose that at time $t=T$, my portfolio has the value:
$$
S_T - FV(S_0) - FV(F)\\
=S_T - S_0e^{(q-r)t} - (S_T - K) = 0
$$
...
$$
This indicates a Markov process, or memory-less process. This indicates that the only relevant variable to the value of $S$ at time $t+dt$ is the value at time $t$. 
